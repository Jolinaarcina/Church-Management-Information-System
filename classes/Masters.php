...
Class Master extends DBConnection {
	...
	function save_entry(){
		if(empty($_POST['id'])){
			...
			$code = sprintf("%'.04d",1);
			while(true){
				$check = $this->conn->query("SELECT * FROM matrimony_list where `code` = '{$prefix}{$code}' ")->num_rows;
				if($check > 0){
		...
		$_POST['wife'] = "{$_POST['wlastname']}, {$_POST['wfirstname']} {$_POST['wmiddlename']}";
		extract($_POST);
		$data = "";
		foreach($_POST as $k =>$v){
			if(in_array($k,array('code','fullname','wife','date','status')) && !is_array($_POST[$k])){
				if(!is_numeric($v))
		...
		if(empty($id)){
			$sql = "INSERT INTO `matrimony_list` set {$data} ";
		}else{
			$sql = "UPDATE `matrimony_list` set {$data} where id = '{$id}' ";
		}
		...
		if($save){
			...
			if(empty($id))
				...
				$data = "";
				foreach($_POST as $k =>$v){
					if(!in_array($k,array('id','code','fullname','wife','date','status')) && !is_array($_POST[$k])){
						if(!is_numeric($v))
							$v = $this->conn->real_escape_string($v);
						if(!empty($data)) $data .=",";
						$data .= " ('{$bid}', '{$k}', '{$v}')";
				...
				if(!empty($data)){
					$sql2 = "INSERT INTO `matrimony_details` (`matrimony_id`,`meta_field`,`meta_value`) VALUES {$data}";
					$this->conn->query("DELETE FROM `matrimony_details` where matrimony_id = '{$bid}'");
					$save_meta = $this->conn->query($sql2);
					if($save_meta){
						$resp['status'] = 'success';
					}else{
						$this->conn->query("DELETE FROM `matrimony_list` where id = '{$bid}'");
						$resp['status'] = 'failed';
				...
				if(isset($witness_name) && $witness_name > 0){
					...
					if(!empty($data)){
						$sql3 = "INSERT INTO `sponsor_list` (`matrimony_id`,`fullname`,`address`) VALUES {$data}";
						$this->conn->query("DELETE FROM `sponsor_list` where matrimony_id = '{$bid}'");
	...
	function delete_entry(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `matrimony_list` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
