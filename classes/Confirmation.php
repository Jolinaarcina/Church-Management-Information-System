...
Class Master extends DBConnection {
	...
	function save_entry(){
		if(empty($_POST['id'])){
			...
			$code = sprintf("%'.04d",1);
			while(true){
				$check = $this->conn->query("SELECT * FROM confirmation_list where `code` = '{$prefix}{$code}' ")->num_rows;
				if($check > 0){
		...
		if(empty($id)){
			$sql = "INSERT INTO `confirmation_list` set {$data} ";
		}else{
			$sql = "UPDATE `confirmation_list` set {$data} where id = '{$id}' ";
		}
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "Confirmation Entry Details has successfully submitted.</b>.";
			else
				$resp['msg'] = "Confirmation Details has been updated successfully.";
				$data = "";
				foreach($_POST as $k =>$v){
				...
				if(!empty($data)){
					$sql2 = "INSERT INTO `confirmation_details` (`confirmation_id`,`meta_field`,`meta_value`) VALUES {$data}";
					$this->conn->query("DELETE FROM `confirmation_details` where confirmation_id = '{$bid}'");
					$save_meta = $this->conn->query($sql2);
					if($save_meta){
						$resp['status'] = 'success';
					}else{
						$this->conn->query("DELETE FROM `confirmation_list` where id = '{$bid}'");
						$resp['status'] = 'failed';
				...
				if(isset($witness_name) && $witness_name > 0){
					...
					if(!empty($data)){
						$sql3 = "INSERT INTO `cwitness_list` (`confirmation_id`,`fullname`,`address`) VALUES {$data}";
						$this->conn->query("DELETE FROM `cwitness_list` where confirmation_id = '{$bid}'");
	...
	function delete_entry(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `confirmation_list` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
