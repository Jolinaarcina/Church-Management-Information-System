...
Class Master extends DBConnection {
	...
	function save_entry(){
		if(empty($_POST['id'])){
			...
			$code = sprintf("%'.04d",1);
			while(true){
				$check = $this->conn->query("SELECT * FROM brgy_list where `code` = '{$prefix}{$code}' ")->num_rows;
				if($check > 0){
		...
		foreach($_POST as $k =>$v){
			if(in_array($k,array('code','brgy')) && !is_array($_POST[$k])){
				if(!is_numeric($v))
		...
		if(empty($id)){
			$sql = "INSERT INTO `brgy_list` set {$data} ";
		}else{
			$sql = "UPDATE `brgy_list` set {$data} where id = '{$id}' ";
		}
		...
		if($save){
			...
			if(empty($id))
				...
				$data = "";
				foreach($_POST as $k =>$v){
					if(!in_array($k,array('id','code','brgy')) && !is_array($_POST[$k])){
						if(!is_numeric($v))
							$v = $this->conn->real_escape_string($v);
						if(!empty($data)) $data .=",";
						$data .= " ('{$bid}', '{$k}', '{$v}')";
	...
	function delete_entry(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `brgy_list` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
