...
Class Master extends DBConnection {
	...
	function save_entry(){
		if(empty($_POST['id'])){
			...
			$code = sprintf("%'.04d",1);
			while(true){
				$check = $this->conn->query("SELECT * FROM members_list where `code` = '{$prefix}{$code}' ")->num_rows;
				if($check > 0){
		...
		foreach($_POST as $k =>$v){
			if(in_array($k,array('code','fullname','brgy','telephone')) && !is_array($_POST[$k])){
				if(!is_numeric($v))
		...
		if(empty($id)){
			$sql = "INSERT INTO `members_list` set {$data} ";
		}else{
			$sql = "UPDATE `members_list` set {$data} where id = '{$id}' ";
		}
		...
		if($save){
			...
			$resp['status'] = 'success';
			if(empty($id))
				$resp['msg'] = "Member Entry Details has successfully submitted.</b>.";
			else
				$resp['msg'] = "Member Details has been updated successfully.";
				$data = "";
				foreach($_POST as $k =>$v){
					if(!in_array($k,array('id','code','fullname','brgy','telephone')) && !is_array($_POST[$k])){
						if(!is_numeric($v))
							$v = $this->conn->real_escape_string($v);
						if(!empty($data)) $data .=",";
						$data .= " ('{$bid}', '{$k}', '{$v}')";
				...
				if(!empty($data)){
					$sql2 = "INSERT INTO `members_details` (`members_id`,`meta_field`,`meta_value`) VALUES {$data}";
					$this->conn->query("DELETE FROM `members_details` where members_id = '{$bid}'");
					$save_meta = $this->conn->query($sql2);
					if($save_meta){
						$resp['status'] = 'success';
					}else{
						$this->conn->query("DELETE FROM `members_list` where id = '{$bid}'");
						$resp['status'] = 'failed';
	...
	function delete_entry(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `members_list` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
